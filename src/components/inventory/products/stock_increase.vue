<template>
    <ValidationObserver ref="form" v-slot="{handleSubmit, invalid}">
        <b-form @submit.prevent="handleSubmit(onSubmit)" autocomplete="off">
            <b-modal visible no-close-on-esc no-close-on-backdrop title="Increase Stock" body-class="p-0">
                <overlay :show="showLoader">
                    <template v-if="!showLoader">
                        ...
                        <b-row v-else class="p-3">
                            ...
                            <b-col md="9">
                                <b-form-group label="Increase reason" label-for="reason">
...
<script>
    ...
    export default {
        ...
        created: async function () {
            ...
            const increaseStockReasonsResponse = await this.$http.get("utilities/increase-stock-reasons")
            this.reasons = increaseStockReasonsResponse.data
        ...
        methods: {
            onSubmit: function () {
                ...
                this.$http.post("inventory/products/stocks/increase/" + this.branch_id + '/' + this.$route.params.id, this.form)
                .then(res => {
