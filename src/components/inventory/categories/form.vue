<template>
    <ValidationObserver ref="form" v-slot="{handleSubmit, invalid}">
        <b-form @submit.prevent="handleSubmit(onSubmit)" autocomplete="off">
            <b-modal :visible=true no-close-on-esc no-close-on-backdrop @hidden="$router.push({name: 'inventory.categories'})">
                <template #modal-title>
                    {{$route.params.id ? "Edit Category" : "New Category"}}
                </template>
                <ValidationProvider vid="name" name="Category name" :rules="{required: true}" v-slot="{errors}">
                    <b-form-group label="Category name *" label-for="name">
                        <b-form-input v-model="form.name" :class="{'is-invalid': errors[0]}" placeholder="e.g. Hair Products" />
                        <b-form-invalid-feedback v-if="errors[0]" v-html="errors[0]"></b-form-invalid-feedback>
                    </b-form-group>
                </ValidationProvider>
                <template #modal-footer>
                    <div class="d-flex flex-grow-1 justify-content-between">
                        <router-link v-if="$route.params.id" :to="{name: 'inventory.categories.delete', params: {id: $route.params.id, name: form.name}}" class="mr-auto btn btn-danger">DELETE</router-link>
    ...
    export default {
        ...
        methods: {
            onSubmit: function () {
                ...
                if (this.$route.params.id) {
                    axios = this.$http.put("inventory/categories/" + this.$route.params.id, this.form)
                } else {
                    axios = this.$http.post("inventory/categories", this.form)
                }
