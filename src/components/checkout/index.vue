        ...
        <overlay :show="showDataLoadingLoader" full-height>
            <template v-if="!showDataLoadingLoader">
                <overlay :show="showSaveInvoiceLoader">
                    ...
                    <b-row>
                        <b-col md="9" :class="{'d-flex left-side p-0' : true}">
                            <overlay :class="{'d-flex flex-grow-1' : true, 'p-4' : $screen.width <= 992 }" :show="client.showDetails" variant="secondary">
                                <template #overlay>
                                ...
                                <div v-else :class="{'d-flex align-items-start flex-grow-1 scrollable' : true , 'pt-5' : $screen.width > 992 }">
                                    <div class="wrapper p-2">
                                        <ValidationObserver ref="form">
                                            <b-form autocomplete="off">
                                                <b-card v-for="item, idx in invoice.items" v-bind:key="'item-' + idx" :class="{'mt-5': idx > 0}">
                                                    <b-row>
                                                        <b-col :class="{'col-sm-7' : $screen.width > 420}">
                                                            <p class="m-0 font-size-17 font-weight-700">{{item.title}}</p>
                                                            <p class="m-0 mt-1 font-size-15 sub-color description" v-html="item.description"></p>
                                                        </b-col>
                                                        <b-col class="text-right">
                                                            <template v-if="item.special_price > 0">
                                                                <p class="m-0 font-size-17 font-weight-700">
                                                                    {{$store.getters.user.business.country.currency}} {{item.special_price * (item.quantity - item.deal_quantity >= 0 ? item.quantity - item.deal_quantity : 0) | currency}}
                                                                </p>
                                                                <del class="sub-color">
                                                                    {{$store.getters.user.business.country.currency}} {{item.original_price * (item.quantity - item.deal_quantity >= 0 ? item.quantity - item.deal_quantity : 0) | currency}}
                                                                </del>
                                                            </template>
                                                            <p v-else class="m-0 font-size-17 font-weight-700">
                                                                {{$store.getters.user.business.country.currency}} {{item.price * (item.quantity - item.deal_quantity >= 0 ? item.quantity - item.deal_quantity : 0) | currency}}
                                                            </p>
                                                        </b-col>
                                                        <b-col :class="{'text-right pl-0' : true, 'col-sm-1' : $screen.width > 420}">
                                                            <b-icon-x variant="danger" font-scale="1.5" class="pointer" @click="invoice.items.splice(idx, 1)"></b-icon-x>
                                                        </b-col>
                                                    </b-row>
                                                    <b-row class="mt-3 qty-prc-row">
                                                        <b-col :class="{'col-2' : $screen.width > 992}">
                                                            <b-form-group label="Qty" label-for="qty">
                                                                <template v-if="['service'].includes(item.type)">
                                                                    <div class="form-control">{{item.quantity}}</div>
                                                                </template>
                                                                <b-form-input v-else @change="calculateItemPrice(item)" type="number" v-model="item.quantity"></b-form-input>
                                                            </b-form-group>
                                                        </b-col>
                                                        <b-col>
                                                            <b-form-group label="Price" label-for="price">
                                                                <b-input-group>
                                                                    <template #prepend>
                                                                        <b-input-group-text class="pt-0 pb-0 pl-2 pr-2">{{$store.getters.user.business.country.currency}}</b-input-group-text>
                                                                    </template>
                                                                    <template v-if="!!item.discount_id || item.type === 'deal' || item.client_deal_id">
                                                                        <div class="form-control">{{item.price}}</div>
                                                                    </template>
                                                                    <b-form-input @change="calculateItemPrice(item)" v-else type="number" v-model="item.price" :max="item.original_price"></b-form-input>
                                                                </b-input-group>
                                                            </b-form-group>
                                                        </b-col>
                                                        <b-col>
                                                        ...
                                                        <b-col v-if="item.type !== 'deal'">
                                                            <b-form-group label="Discount" label-for="discount">
                                                                <div v-if="(item.original_price - item.price > 0)" class="form-control">
                                                                    Manual Discount {{$store.getters.user.business.country.currency}} {{item.original_price - item.price | currency}}
                                                                </div>
                                                                <template v-else>
                                                                    <b-form-select v-if="!item.client_deal_id" @change="calculateItemPrice(item)" :options="item.discountOptions" value-field="id" v-model="item.discount_id">
                                                                        <template #first>
                                                                            <b-form-select-option :value="null">No discount</b-form-select-option>
                                                                        </template>
                                                                    </b-form-select>
                                                                    <div v-else class="form-control">No Discount</div>
                                                                </template>
                                                            </b-form-group>
                                                        </b-col>
                                                    </b-row>
                                                    <b-form-group v-if="item.clientDealsOptions.length" :label="'Select Deal' + (item.client_deal_id ? ' - (Remaining Qty: ' + parseInt(item.deal_quantity - item.quantity >= 0 ? item.deal_quantity - item.quantity : 0) + ')' : '')" label-for="deal">
                                                        <b-form-select @change="calculateItemPrice(item)" :options="item.clientDealsOptions" v-model="item.client_deal_id">
                                                            <template #first>
                                                                <b-form-select-option :value="null">-- Please Select --</b-form-select-option>
                                                            </template>
                                                        </b-form-select>
                                        ...
                                        <b-row class="mt-4">
                                            ...
                                            <b-col md="5">
                                                ...
                                                <div v-for="tax, idx in taxes" v-bind:key="'tax-' + idx" class="d-flex justify-content-between font-size-17">
                                                    ...
                                                    <p class="m-0">{{$store.getters.user.business.country.currency}} {{tax.amount * tax.quantity | currency}}</p>
                                                </div>
                                                ...
                                                <div :class="{'d-flex justify-content-between font-weight-700 border-top mt-3 pt-3 pb-3' : true, 'font-size-20' : $screen.width > 620, 'font-size-17' : $screen.width <= 620}">
                        ...
                        <b-col md="3" :class="{'right-side bg-white border-left p-0 d-flex flex-column' : true, 'pb-5 mb-3' : $screen.width <= 1024}">
                            <div class="d-flex flex-column flex-grow-1 align-items-center justify-content-center">
                                ...
                                <div v-else class="flex-grow-1 d-flex flex-column align-items-center justify-content-center text-center p-2">
                                ...
                                <div v-if="invoice.items.length && !client.showDetails" class="p-3 border-top w-100">
                                    <div class="d-flex justify-content-center flex-wrap" v-if="!invoice.payment_method_id">
                                        <div class="pr-1 pl-1 mb-2" style="width:105px" v-for="paymentMethod in paymentMethods" v-bind:key="'payment-method-' + paymentMethod.id">
                            ...
                            <div v-if="!client.showDetails && !invoice.payment_method_id" class="p-3 border-top d-flex flex-wrap justify-content-center">
                                <dropdown dropup block text="More Options" style="width:200px" class="box-shadow with-border" menu-class="w-100" variant="outline-transparent" :disabled="!invoice.items.length">
                    ...
                    <items-modal ref="items-modal" :items="['services', 'products', 'deals']" :service-url-params="{branch_id: $route.query.branch_id, group_by: 'category_hierarchy'}" :product-url-params="{branch_id: $route.query.branch_id, group_by: 'category_hierarchy', stock_available: true}" :deal-url-params="{'with-inclusions': true, 'purchasable': true, 'with-tax': true}" @selected="onItemSelected($event)"></items-modal>
    ...
    export default {
        ...
        created: async function () {
            if (this.$route.query.client_id) {
                ...
                this.client.data = clientResponse.data
            }
            
            const staffPromise = new Promise((resolve) => { this.$http.get("staff", {params: {branch_ids: this.$route.query.branch_id}}).then(response => resolve(response)) })
        ...
        computed: {
            ...
            taxes: function () {
                ...
                this.invoice.items.forEach(item => {
                    item.taxes.forEach(tax => {
                        let taxRow = taxes.find(t => t.id == tax.id)
                        const quantity = (item.quantity - item.deal_quantity >= 0 ? item.quantity - item.deal_quantity : 0)
                        if (!taxRow) {
                            taxes.push(Object.assign(tax, {quantity: quantity}))
                        } else {
                            taxRow.quantity = quantity
                        }
                    })
                })
                return taxes
            ...
            subTotal: function () {
                let subTotal = 0
                this.invoice.items.forEach(item => {
                    subTotal += parseFloat((item.special_price > 0 ? item.special_price : item.price) * (item.quantity - item.deal_quantity >= 0 ? item.quantity - item.deal_quantity : 0))
                })
                if (subTotal > 0) {
            ...
            taxTotal: function () {
                ...
                this.taxes.forEach(tax => {
                    taxTotal += tax.amount * tax.quantity
                })
                return taxTotal
            },
            grandTotal: function () {
                return this.subTotal > 0 ? this.subTotal + this.taxTotal : 0
            }
        },
        methods: {
            ...
            addClientDeals: function (item) {
                if (this.client.data.deals) {
                    this.client.data.deals.forEach(clientDeal => {
                        if (item.type === "service") {
                            const clientDeals = clientDeal.items.filter(dealItem => dealItem.service_id == item.id)
                            if (clientDeals) {
                                clientDeals.forEach(dealItem => {
                                    item.clientDealsOptions.push({value: clientDeal.id, text: clientDeal.deal.name, quantity_available: dealItem.quantity_available - dealItem.quantity_utilized})
                                })
                            }
                        } else if (item.type === "product") {
                            const clientDeals = clientDeal.items.filter(dealItem => dealItem.product_id == item.id)
                            if (clientDeals) {
                                clientDeals.forEach(dealItem => {
                                    item.clientDealsOptions.push({value: clientDeal.id, text: clientDeal.deal.name, quantity_available: dealItem.quantity_available - dealItem.quantity_utilized})
                                })
            ...
            onItemSelected: async function ($event) {
                if ($event.type === "service") {
                    const isAlreadyAdded = this.invoice.items.find(item => item.type === "service" && item.id == $event.data.id)
                    if (isAlreadyAdded) {
                        this.$helpers.showToast("danger", "Service already added")
                    } else {
                        this.invoice.items.push({
                            type: $event.type,
                            id: $event.data.service.id,
                            title: $event.data.service.title,
                            description: this.$helpers.convertToHoursAndMinutes($event.data.pricing.duration),
                            quantity: 1,
                            deal_quantity: 0,
                            original_price: $event.data.pricing.price,
                            price: $event.data.pricing.price,
                            special_price: $event.data.pricing.special_price,
                            discount: 0,
                            staff_id: $event.staff_id ?? (this.$route.query.staff_id ?? this.$store.getters.user.staff.id),
                            discount_id: null,
                            client_deal_id: null,
                            taxes: [],
                            clientDealsOptions: [],
                    ...
                    const isAlreadyAdded = this.invoice.items.find(item => item.type === "product" && item.id == $event.data.id)
                    if (isAlreadyAdded) {
                        this.$helpers.showToast("danger", "Product already added")
                    } else {
                        this.invoice.items.push({
                            type: $event.type,
                            id: $event.data.id,
                            title: $event.data.name,
                            description: $event.data.stock_on_hand + " in hand",
                            quantity: 1,
                            deal_quantity: 0,
                            original_price: $event.data.retail_price,
                            price: $event.data.retail_price,
                            special_price: $event.data.special_price,
                            discount: 0,
                            staff_id: $event.staff_id ?? (this.$route.query.staff_id ?? this.$store.getters.user.staff.id),
                            discount_id: null,
                            client_deal_id: null,
                    ...
                    const isAlreadyAdded = this.invoice.items.find(item => item.type === "deal" && item.id == $event.data.id)
                    if (isAlreadyAdded) {
                        this.$helpers.showToast("danger", "Deal already added")
                    } else {
                        this.invoice.items.push({
                            type: $event.type,
                            id: $event.data.id,
                            ...
                            quantity: 1,
                            deal_quantity: 0,
                            original_price: $event.data.price,
                ...
                let item = this.invoice.items[this.invoice.items.length - 1]
                item.discountOptions = []
                if (item.special_price > 0) {
                    const discountPrice = item.price - item.special_price
                    item.discount_id = "special"
                    item.discount = item.price - item.special_price
                    item.discountOptions.push({
                        id: item.discount_id,
                        type: "amount",
                        value: discountPrice,
                ...
                if (item.type === "service") {
                    item.discountOptions = item.discountOptions.concat(this.discounts.filter(discountTypeRow => discountTypeRow.enable_for_service))
                    const taxPromise = await this.$http.get("taxes/get_by_service?branch_id=" + this.$route.query.branch_id + "&service_id=" + item.id)
                    taxResponse = taxPromise.data
                } else if (item.type === "product") {
                    item.discountOptions = item.discountOptions.concat(this.discounts.filter(discountTypeRow => discountTypeRow.enable_for_product))
                    const taxPromise = await this.$http.get("taxes/get_by_product?branch_id=" + this.$route.query.branch_id + "&product_id=" + item.id)
                    taxResponse = taxPromise.data
                } else if (item.type === "deal") {
                    taxResponse.tax = item.tax
                }

                if (taxResponse.tax) {
                    if (taxResponse.tax.rate) {
                        item.taxes.push({ id: taxResponse.tax.id, title: taxResponse.tax.title, rate: taxResponse.tax.rate, amount: this.$helpers.calculateTax(taxResponse.tax.rate, (item.special_price > 0 ? item.special_price : item.price) * item.quantity) })
                    } else {
                        if (taxResponse.tax.tax_1) {
                            item.taxes.push({ id: taxResponse.tax.tax_1.id, title: taxResponse.tax.tax_1.title, rate: taxResponse.tax.tax_1.rate, amount: this.$helpers.calculateTax(taxResponse.tax.tax_1.rate, (item.special_price > 0 ? item.special_price : item.price) * item.quantity) })
                        }
                        if (taxResponse.tax.tax_2) {
                            item.taxes.push({ id: taxResponse.tax.tax_2.id, title: taxResponse.tax.tax_2.title, rate: taxResponse.tax.tax_2.rate, amount: this.$helpers.calculateTax(taxResponse.tax.tax_2.rate, (item.special_price > 0 ? item.special_price : item.price) * item.quantity) })
                        }
                        if (taxResponse.tax.tax_3) {
                            item.taxes.push({ id: taxResponse.tax.tax_3.id, title: taxResponse.tax.tax_3.title, rate: taxResponse.tax.tax_3.rate, amount: this.$helpers.calculateTax(taxResponse.tax.tax_3.rate, (item.special_price > 0 ? item.special_price : item.price) * item.quantity) })
                        }
                    }
                }
            ...
            calculateItemPrice: function (item) {
                ...
                item.deal_quantity = 0
                item.discount = 0

                if (item.client_deal_id) {
                    const clientDeal = item.clientDealsOptions.find(clientDeal => clientDeal.value == item.client_deal_id)
                    item.deal_quantity = clientDeal.quantity_available
                ...
                if (item.discount_id) {
                    ...
                    if (discountOption.type === "amount") {
                        item.special_price = item.price - discountOption.value
                    } else {
                ...
                item.taxes.forEach(tax => {
                    tax.amount = this.$helpers.calculateTax(tax.rate, (item.special_price > 0 ? item.special_price : item.price))
                })
                ...
                this.$refs.form.validate().then(isValid => {
                    if (isValid) {
                        ...
                        if (this.invoice.items.filter(item => item.type === "deal").length) {
                            ...
                            if (!this.invoice.payment_method_id) {
                                this.$helpers.showToast("danger", "You cannot save an invoice as unpaid if you're selling a deal")
                                return
                            }
                        }

                        let invoiceData = {...this.invoice}
                        invoiceData.branch_id = this.$route.query.branch_id
                        invoiceData.subtotal = this.subTotal
                        invoiceData.tax = this.taxTotal
                        invoiceData.grandtotal = this.grandTotal
                        invoiceData.client_id = this.client.data.id
                        invoiceData.appointment_id = this.$route.query.appointment_id
