<template>
    <b-container fluid>
        <overlay :show="showInvoiceLoader || showSaveInvoiceLoader" full-height>
                            ...
                            <div class="wrapper">
                                <b-card>
                                    <div class="text-center">
                                        <p class="m-0 font-size-20 font-weight-700">Invoice #{{invoice.invoice_number}}</p>
                                        <p class="m-0 mt-1 sub-color font-size-15">{{$helpers.formatDate(invoice.created_at)}}</p>
                                    </div>
                                    <div v-for="item in invoice.items" v-bind:key="'item-' + item.id" class="d-flex flex-column">
                                        <p class="m-0 sub-color font-size-15">
                                            {{item.quantity}}
                                            <template v-if="item.quantity > 1">items</template>
                                            <template v-else>item</template>
                                        </p>
                                        <div class="d-flex justify-content-between">
                                            <div class="d-flex flex-column">
                                                <router-link v-if="item.product_id" :to="{name: 'inventory.products.stock-history', params: {id: item.product_id}}" class="font-size-17">{{item.title}}</router-link>
                                                <p v-else class="m-0 font-size-17">{{item.title}}</p>
                                                <p v-for="discount in item.discounts" v-bind:key="'discount' + discount.id" class="m-0 mt-1 sub-color font-size-15">
                                                    {{discount.title}}
                                                </p>
                                            </div>
                                            <div class="d-flex flex-column text-right">
                                                <template v-if="item.discount > 0">
                                                    <p class="m-0 font-size-17">{{$store.getters.user.business.country.currency}} {{(item.price - item.discount) * item.quantity | currency}}</p>
                                                    <del class="mt-1 sub-color font-size-15">{{$store.getters.user.business.country.currency}} {{item.price * item.quantity | currency}}</del>
                                                </template>
                                                <p v-else class="m-0 font-size-17">
                                    ...
                                    <div class="d-flex justify-content-between border-top mt-4 pt-4">
                                        <p class="m-0 font-size-17">Subtotal</p>
                                        <p class="m-0 font-size-17">{{$store.getters.user.business.country.currency}} {{invoice.subtotal | currency}}</p>
                                    </div>
                                    <div v-for="tax in taxes" v-bind:key="'tax-' + tax.id" class="d-flex justify-content-between mt-2">
                                        <p class="m-0 font-size-17">{{tax.title}} - {{tax.rate}}%</p>
                                        <p class="m-0 font-size-17">{{$store.getters.user.business.country.currency}} {{tax.amount | currency}}</p>
                                    </div>
                                    <div class="border-top pt-4 mt-4 d-flex justify-content-between">
                                        <p class="m-0 font-size-20 font-weight-700">Total</p>
                                        <p class="m-0 font-size-20 font-weight-700">{{$store.getters.user.business.country.currency}} {{invoice.grandtotal | currency}}</p>
                                    </div>
                                    <div v-if="invoice.payment_method_id" class="d-flex justify-content-between border-top mt-4 pt-4">
                                        <p class="m-0 font-size-17">
                                            {{paymentMethods.find(paymentMethod => paymentMethod.id == invoice.payment_method_id).title}}
                                        </p>
                                        <p class="m-0 font-size-17">{{$store.getters.user.business.country.currency}} {{invoice.grandtotal | currency}}</p>
                                    </div>
                                    <div class="border-top pt-4 mt-4 d-flex justify-content-between">
                                        <p class="m-0 font-size-17">Balance</p>
                                        <p class="m-0 font-size-17">{{$store.getters.user.business.country.currency}} {{invoice.balance | currency}}</p>
                                    </div>
                                    <div v-if="invoice.notes" class="mt-4 sub-color font-italic">{{invoice.notes}}</div>
                                </b-card>
                            </div>
                        </div>
                    </b-col>
    ...
    export default {
                ...
                if (this.invoice.items) {
                    this.invoice.items.forEach(item => {
                        item.taxes.forEach(tax => {
                            let taxRow = taxes.find(taxRow => taxRow.id == tax.id)
                            const taxAmount = tax.amount * item.quantity
                            if (taxRow) {
                                taxRow.amount += taxAmount
                            } else {
                                taxes.push({ title: tax.title, rate: tax.rate, amount: taxAmount })
                            }
                        })
                    })
                }
        ...
        methods: {
            getInvoice: async function () {
                ...
                const response = await this.$http.get("invoices/" + this.$route.params.id + "?with-items&with-item-taxes&with-client&with-branch&with-payment-method&with-refund-invoice&with-original-invoice")
